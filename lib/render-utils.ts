...
export function drawTrapezoidal(
  ...
  try {
    const transformer = getSharedGpu();
    if (transformer) {
      const forceNorm = Math.max(-0.5, Math.min(0.5, force / 100));
      const htmlSource = source instanceof HTMLCanvasElement ? source : (() => {
        const tmp = document.createElement('canvas');
        tmp.width = source.width;
        tmp.height = source.height;
        const tCtx = tmp.getContext('2d');
        if (tCtx) tCtx.drawImage(source as any, 0, 0);
        return tmp;
      })();
      const result = transformer.render(htmlSource, {
        mode: 0,
        angle: st.angle,
        force: forceNorm,
      });
...
export function renderSpecToOffscreen(
  ...
  spec.symbols.forEach((sym) => {
    ...
    // scale/bounds/offset are Grid Space — already baked into x1/y1/sw/sh, must NOT reach glyph renderer
    const cleanSym = {
      ...sym,
      st: (hasSt ? sym.st : undefined),
      sp: (hasSp ? sym.sp : undefined),
      // Grid Space props — baked into start/end, must NOT affect Symbol rendering
      scale: undefined,
      bounds: undefined,
      offset: undefined,
